...
void rotate(Node* u) {
    Node *v = u->parent, *w = v->parent;
    bool u_lr = u == v->child[1], v_lr = v == w->child[1];
    Node* t = u->child[!u_lr];
    w->child[v_lr] = u;
    u->child[!u_lr] = v;
    v->child[u_lr] = t;
...
void update_and_balance(Node* u) {
    while (u != dummyptr) {
        bool higher_lr = u->child[0]->height < u->child[1]->height;
        if (u->child[higher_lr]->height - u->child[!higher_lr]->height > 1) {
            u = u->child[higher_lr];
            bool sub_higher_lr = u->child[0]->height < u->child[1]->height;
            if (u->child[0]->height == u->child[1]->height ||
                higher_lr == sub_higher_lr)
                rotate(u);
            else {
                u = u->child[sub_higher_lr];
                rotate(u);
                rotate(u);
            }
        }
...
void insert(int x) {
    if (tree.root == dummyptr) {
        tree.root = new Node{1, 0, x, x, dummyptr, dummyptr, dummyptr};
        return;
    }
    Node* u = tree.root;
    bool lr = x >= u->s;
    while (u->child[lr] != dummyptr) {
        u = u->child[lr];
        lr = x >= u->s;
    }
    u->child[lr] = new Node{1, 0, x, x, u, dummyptr, dummyptr};
...
void erase(int x) {
    ...
    if (u->child[0] != dummyptr && u->child[1] != dummyptr) {
        v = u;
        u = u->child[1];
        while (u->child[0] != dummyptr)
            u = u->child[0];
        v->s = u->s;
    }
    v = u->parent;
    Node* t = u->child[u->child[1] != dummyptr];
    t->parent = v;
    v->child[v->child[1] == u] = t;
    if (u == tree.root)
        tree.root = t;
    u->child[0] = u->child[1] = dummyptr;
    delete u;
    update_and_balance(v);
}

// Subtask 4: Query the k-th node in the AVL.
int kth(int k) {
    Node* u = tree.root;
    while (k != u->child[0]->size + 1) {
        if (k < u->child[0]->size + 1)
            u = u->child[0];
        else {
            k -= u->child[0]->size + 1;
            u = u->child[1];
...
int ans(int k) {
    Node* u = tree.root;
    int ans = 0;
    int presum = -u->sum, cursum;
    k += 1;
    while (u != dummyptr) {
        cursum = presum + k * (u->child[0]->sum + u->s);
        if (cursum >= 0) {
            ans = cursum;
            u = u->child[0];
        } else {
            presum += k * (u->child[0]->sum + u->s);
            u = u->child[1];
        }
    }
    return ans;
